...
public class MainSecurity extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{//solo se puede acceder a este medodo desde security
        http.cors().and().csrf().disable().authorizeRequests().antMatchers("/api-firstapp/auth/**", "/api-firstapp/contact/**").permitAll().antMatchers("/api-firstapp/category/*").permitAll().antMatchers(HttpMethod.POST, "/api-firstapp/user/").permitAll().antMatchers(HttpMethod.GET, "/api-firstapp/product/*").permitAll().anyRequest().authenticated().and().exceptionHandling().authenticationEntryPoint(entryPoint).and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
